...
public final class EOTD2 {
    ...
    public EOTD2(Text text, int idBytes, int charBytes) {
        int n = text.length();
        this.text = text;
    ...
    private Edge allocate(int pos, int pid, int length) {
        Edge t = free;
        free = t.suf;
        t.pos = pos;
        t.pid = pid;
    ...
    private Edge down(Edge t, int c) {
        ...
        while (true) {
            ...
            if (s.pid == t.pos && text.charAt(s.pos+t.length) == c) return s;
    ...
    private Edge split(Edge f, int c) {
        ...
        Edge g = allocate(f.pos, i-d, f.length);
        f.pos = i-d;
        f.length = d;
        intern[i-d] = f;
    ...
    private Edge addExt(Edge t, int c) {
        ...
        Edge s = allocate(i-d, t.pos, Integer.MAX_VALUE);
        setDown(t, c, s);
        return s;
    }
